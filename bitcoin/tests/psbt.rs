...
use bitcoin::bip32::{DerivationPath, ExtendedPrivKey, ExtendedPubKey, Fingerprint, IntoDerivationPath, KeySource};
...
use bitcoin::{absolute, Address, Amount, Denomination, Network, OutPoint, PrivateKey, PublicKey, ScriptBuf, Sequence, Transaction, transaction, TxIn, TxOut, Witness};
...
#[test]
fn my_test_sign_taproot_in_psbt() {

    macro_rules! test_hex_unwrap (($hex:expr) => (<Vec<u8> as $crate::FromHex>::from_hex($hex).unwrap()));

    {
        //abandon
        //m/86'/1'/0'/0/0  dff1c8c2c016a572914b4c5adb8791d62b4768ae9d0a61be8ab94cf5038d7d90
        //m/86'/1'/0'/0/1  1ede31b0e7e47c2afc65ffd158b1b1b9d3b752bba8fd117dc8b9e944a390e8d9
        //m/86'/1'/0'/0/1  1fb777f1a6fb9b76724551f8bc8ad91b77f33b8c456d65d746035391d724922a
    }
    ...
    //m/86'/1'/0'/0/0
    let sk = "dff1c8c2c016a572914b4c5adb8791d62b4768ae9d0a61be8ab94cf5038d7d90";
    let sk = test_hex_unwrap!(sk);
    ...
    //m/86'/1'/0'/0/1
    let sk = "1ede31b0e7e47c2afc65ffd158b1b1b9d3b752bba8fd117dc8b9e944a390e8d9";
    let sk = test_hex_unwrap!(sk);
    let sk = PrivateKey::from_slice(&sk, Network::Testnet).unwrap();
    let x_only_pubkey = XOnlyPublicKey::from(sk.public_key(secp).inner);
    ...
    // //m/86'/1'/0'/0/2
    let sk = "1fb777f1a6fb9b76724551f8bc8ad91b77f33b8c456d65d746035391d724922a";
    let sk = test_hex_unwrap!(sk);
    let sk = PrivateKey::from_slice(&sk, Network::Testnet).unwrap();
    let x_only_pubkey = XOnlyPublicKey::from(sk.public_key(secp).inner);

    let script3 = script::Builder::new()
        .push_slice(x_only_pubkey.serialize())
        .push_opcode(OP_CHECKSIG)
        .into_script();

    // use public key of path "m/86'/1'/0'/0/2" as internal key
    let internal_key = x_only_pubkey.clone();

    let builder = TaprootBuilder::new();
    let builder = builder.add_leaf(2, script1.clone()).unwrap();
    let builder = builder.add_leaf(2, script2.clone()).unwrap();
    let builder = builder.add_leaf(1, script3.clone()).unwrap();
    ...
    let address = Address::p2tr_tweaked(output_key, Network::Testnet);
    // m/86'/1'/0'/0/7
    let receive_address = "tb1pyfv094rr0vk28lf8v9yx3veaacdzg26ztqk4ga84zucqqhafnn5q9my9rz";
    let receive_address = Address::from_str(receive_address).unwrap();
    ...
    //key path spend
    {
        ...
        let out_puts = vec![
            TxOut { value: send_value, script_pubkey: receive_address.payload.script_pubkey() },
        ];
        ...
        let mut input = Input {
            witness_utxo: {
                ...
                Some(TxOut { value: utxo_value, script_pubkey })
            },
        ...
        let keystore = Keystore {
            mfp: Fingerprint::from_str("73c5da0a").unwrap(),
            sk,
        };
        psbt.sign(&keystore, secp);
        ...
        assert_eq!(sig, psbt.inputs[0].tap_key_sig.unwrap().sig.to_string());
    ...
    // script spend
    {
        ...
        // //m/86'/1'/0'/0/1   signing key
        let sk = "1ede31b0e7e47c2afc65ffd158b1b1b9d3b752bba8fd117dc8b9e944a390e8d9";
        let sk = test_hex_unwrap!(sk);
        let sk = PrivateKey::from_slice(&sk, Network::Testnet).unwrap();
        let x_only_pubkey = XOnlyPublicKey::from(sk.public_key(secp).inner);

        let use_script = script2.clone();
        let use_path = "m/86'/1'/0'/0/1";
        let mut origins = BTreeMap::new();
        origins.insert(
            x_only_pubkey.clone(),
            (
                ...
                (
                    Fingerprint::from_str(mfp).unwrap(),
                    DerivationPath::from_str(use_path).unwrap(),
                ),
            ),
        );
        ...
        tap_scripts.insert(
            tree_info.control_block(&(use_script.clone(), LeafVersion::TapScript)).unwrap(),
        ...
        assert_eq!(sig, psbt.inputs[0].tap_script_sigs.get(&(x_only_pubkey, use_script.clone().tapscript_leaf_hash())).unwrap().sig.to_string());
